...
bool RobotPlanner::planPose(geometry_msgs::PoseStamped pose) {
    // TODO
    lastPlanningTime = (rand()/RAND_MAX)*maxPlanningTime;
...
bool RobotPlanner::place(Part destination) {
    // TODO
    lastPlanningTime = (rand()/RAND_MAX)*maxPlanningTime;
    lastExecutingTime = (rand()/RAND_MAX)*6;
    ros::spinOnce();
...
bool RobotPlanner::move(Part part, Part destination) {
    // TODO
    lastPlanningTime = (rand()/RAND_MAX)*maxPlanningTime;
    lastExecutingTime = (rand()/RAND_MAX)*6;
    ros::spinOnce();
...
void RobotPlanner::sendJointsValue(vector<double> joints) {
    ...
    lastExecutingTime = arrivalTime;
    lastPlanningTime = 0.0;
    ros::spinOnce();
}
